...
public class PlayerOptionPacketListener implements PacketListener {
	...
	private static void option1(Player player, Packet packet) {
		...
		if (player2 == null)
			return;
		player.setWalkToTask(new WalkToAction(player, player2.getPosition(), 1, new Action() {
			@Override
			public void execute() {
				if(player.getLocation() == Location.DEFAULT) {
					if(player.busy()) {
	...
	private static void option2(Player player, Packet packet) {
		...
		if (victim == null)
			return;
		/*player.setWalkToTask(new WalkToTask(player, victim.getPosition(), 1, new Action() {
			@Override
			public void execute() {
