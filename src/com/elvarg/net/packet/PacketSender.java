...
import com.elvarg.game.content.skill.construction.Construction;
...
public class PacketSender {
	...
	public PacketSender alterGroundItem(ItemOnGround item) {
		...
		out.put(0);
		out.putShort(item.getItem().getId()).putShort(item.getOldAmount()).
		putShort(item.getItem().getAmount());
		player.getSession().write(out);
		return this;
	}
	...
	public PacketSender createGroundItem(ItemOnGround item) {
		...
		out.putShort(item.getItem().getAmount()).put(0);
		player.getSession().write(out);
		return this;
	}
