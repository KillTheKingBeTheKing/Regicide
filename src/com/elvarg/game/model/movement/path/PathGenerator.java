...
public class PathGenerator {
	...
	public static Position getCombatPath(Character character, Position c, int size) {
	...
	public static Position getBasicPath(Character character, Position c) {
		int x = c.getX();
		int y = c.getY();
		int z = c.getZ();
		...
		for (int i = 0; i < nodes.length; i++) {
			if (isInteractionPathClear(nodes[i][0], nodes[i][1], z, x, y)) {
				...
				if ((dist < bestDist) && (RegionManager.canMove(new Position(character.getPosition().getX(), character.getPosition().getY(), character.getPosition().getZ()), NON_DIAGONAL_DIRECTIONS[i]))) {
					bestDist = dist;
					bestX = nodes[i][0];
					bestY = nodes[i][1];
				}
