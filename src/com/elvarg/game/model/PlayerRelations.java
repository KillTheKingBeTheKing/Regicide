...
public class PlayerRelations {
	 ...
	 */
	public void message(Optional<Player> friend, String message) {
		if(!friend.isPresent() || message == null) {
			this.player.getPacketSender().sendMessage("This player is currently offline.");
			return;
		}
		if (friend.get().getRelations().status.equals(PrivateChatStatus.FRIENDS_ONLY) && !friend.get().getRelations().friendList.contains(player.getLongUsername()) || friend.get().getRelations().status.equals(PrivateChatStatus.OFF)) {
			this.player.getPacketSender().sendMessage("This player is currently offline.");
			return;
		}
		if(status == PrivateChatStatus.OFF) {
		...
		friend.get().getPacketSender().sendPrivateMessage(player.getLongUsername(), player.getRights(), message);
	}
