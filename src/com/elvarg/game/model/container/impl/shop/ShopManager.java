package com.elvarg.game.model.container.impl.shop;
...
public class ShopManager extends ShopIdentifiers {
	 ...
	 * 
	 * @param player
	 *            The player pricechecking.
	 * @param itemId
	 *            The item id to price check.
	 * @param slot
	 *            The item's slot.
	 * @param shopItem
	 *            Are we pricechecking a shop item or player item?
	 */
	...
	private static int getItemValue(Player player, ItemDefinition itemDef, int shopId) {
		...
		// Custom prices example
		/*if (shopId == POINTS_SHOP) {
			switch (itemDef.getId()) {
			case ItemIdentifiers.DRAGON_SCIMITAR:
				return 100; // 100 points cost
			}
		}*/
		
		// Returns the item's definition value as
		// default
