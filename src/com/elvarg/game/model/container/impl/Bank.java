...
import com.elvarg.game.model.syntax.impl.SearchBank;
...
public class Bank extends ItemContainer {
	...
	public Bank open() {
		...
		getPlayer().getPacketSender().sendConfig(115, getPlayer().withdrawAsNote() ? 1 : 0)
				...
				.sendConfig(117, getPlayer().isSearchingBank() ? 117 : 0).sendInterfaceSet(5292, 5063);
	...
	public Bank refreshItems() {
		...
		if (getPlayer().isSearchingBank()) {
			...
			getPlayer().getPacketSender().sendString(5383, "Bank of Regicide").sendConfig(117, 0);
		}
	...
	public Bank switchItem(ItemContainer to, Item item, int slot, boolean sort, boolean refresh) {
		...
		if (to instanceof Inventory) {
			/*
			 * boolean withdrawAsNote = getPlayer().withdrawAsNote() && def != null &&
			 * def.isNoted() && item.getDefinition() != null &&
			 * def.getName().equalsIgnoreCase(item.getDefinition().getName()) &&
			 * !def.getName().contains("Torva") && !def.getName().contains("Virtus") &&
			 * !def.getName().contains("Pernix") && !def.getName().contains("Torva"); int
			 * checkId = withdrawAsNote ? item.getId() + 1 : item.getId();
