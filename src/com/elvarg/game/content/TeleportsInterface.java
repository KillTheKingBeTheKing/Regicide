...
public class TeleportsInterface {
	...
	private enum TeleportsData {
		...
		// Wildy teles
		DITCH(38600, 38602, new Position(3089, 3523)), WEST_DRAGONS(38600, 38606, new Position(2978, 3598)), OBELISK(
				38600, 38610, new Position(3156, 3615)), GRAVEYARD(38600, 38614,
						new Position(3148, 3671)), BANDIT_CAMP(38600, 38618,
								new Position(3034, 3690)), HUNTERS_HILL(38600, 38622, new Position(3143,
										3773)), DEMONIC_RUINS(38600, 38626, new Position(3283, 3877)), RUNITE_ROCKS(
												38600, 38630, new Position(3059, 3884)), THE_GATE(38600, 38634,
														new Position(3225, 3903)), TARGET_TELEPORT(38600, 38638, null),

		// Boss teles
		VENENATIS(38300, 38302, new Position(3239, 3734)), CALLISO(38300, 38306,
				new Position(3307, 3837)), CHAOS_ELEMENTAL(38300, 38310, new Position(3267, 3912)),
					...
					public void handleOption(Player player, int option) {
						...
						if (option == 1) {
							if (TeleportHandler.checkReqs(player, target.get().getPosition())) {
								TeleportHandler.teleport(player, target.get().getPosition().copy(),
										TeleportType.NORMAL);
							}
						}
					}
