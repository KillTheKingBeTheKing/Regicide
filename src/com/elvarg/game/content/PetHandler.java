...
/**
 * Handles pets. Allows them to be dropped on the ground aswell as picked
 * up. Credits to Lumiere from R-S for the huge {@link Pet} data.
...
public class PetHandler {
		...
		for(Pet pet : Pet.SKILLING_PETS) {
			if(pet.getSkill().isPresent() && pet.getSkill().get() == skill) {
				if(Misc.getRandom(pet.getChance()) == 1) {
					World.sendMessage("@dre@"+player.getUsername()+" just found a stray "+pet.getName()+" while "+Misc.formatName(skill.toString().toLowerCase())+"!");
	...
	public static boolean drop(Player player, int id, boolean reward) {
		...
		if(pet.isPresent()) {
			...
			if(player.getCurrentPet() == null) {
				...
				//Spawn the pet..
				NPC npc = new NPC(pet.get().getId(), player.getPosition().copy().add(0, 1));
				npc.setOwner(player);
				...
				if(reward) {
					player.getPacketSender().sendMessage("@dre@You suddenly get a weird feeling, as if someone is following you.");
				} else {
	 ...
	 * @param player		The player morphing a pet.
	 * @param npc			The pet being morphed.
	 * @return
	 */
	...
	public enum Pet {
		...
		HELLPUPPY(964, 0, 13247, 138) {
			...
			public int getDialogue(Player player) {
				...
				return dialogueIds[Misc.getRandom(dialogueIds.length)];
			}
		},
		...
		//SKILL PETS
		HERON(6715, 0, 13320, -1, Skill.FISHING, 5000),
		BEAVER(6717, 0, 13322, -1, Skill.WOODCUTTING, 5000),
		GREY_CHINCHOMPA(6719, 6720, 13324, -1, Skill.HUNTER, 3000),
		RED_CHINCHOMPA(6718, 6719, 13323, -1, Skill.HUNTER, 4000),
		BLACK_CHINCHOMPA(6720, 6718, 13325, -1, Skill.HUNTER, 5000),
		ROCK_GOLEM(6723, 0, 13321, -1, Skill.MINING, 5000),
		GIANT_SQUIRREL(7334, 0, 20659, -1, Skill.AGILITY, 5000),
		TANGLEROOT(7335, 0, 0, -1, Skill.FARMING, 5000),
		ROCKY(7336, 0, 0, -1, Skill.THIEVING, 5000),

		//RIFT GUARDIANS (SKILL PETS)
		FIRE_RIFT_GAURDIAN(7337, 7338, 20665, -1, Skill.RUNECRAFTING, 8000),
		AIR_RIFT_GUARDIAN(7338, 7339, 20667, -1, Skill.RUNECRAFTING, 8000),
		MIND_RIFT_GUARDIAN(7339, 7340, 20669, -1, Skill.RUNECRAFTING, 8000),
		WATER_RIFT_GUARDIAN(7340, 7341, 20671, -1, Skill.RUNECRAFTING, 8000),
		EARTH_RIFT_GUARDIAN(7341, 7342, 20673, -1, Skill.RUNECRAFTING, 8000),
		BODY_RIFT_GUARDIAN(7342, 7343, 20675, -1, Skill.RUNECRAFTING, 8000),
		COSMIC_RIFT_GUARDIAN(7343, 7344, 20677, -1, Skill.RUNECRAFTING, 8000),
		CHAOS_RIFT_GUARDIAN(7344, 7345, 20679, -1, Skill.RUNECRAFTING, 8000),
		NATURE_RIFT_GUARDIAN(7345, 7346, 20681, -1, Skill.RUNECRAFTING, 8000),
		LAW_RIFT_GUARDIAN(7346, 7347, 20683, -1, Skill.RUNECRAFTING, 8000),
		DEATH_RIFT_GUARDIAN(7347, 7348, 20685, -1, Skill.RUNECRAFTING, 8000),
		SOUL_RIFT_GUARDIAN(7348, 7349, 20687, -1, Skill.RUNECRAFTING, 8000),
		ASTRAL_RIFT_GUARDIAN(7349, 7350, 20689, -1, Skill.RUNECRAFTING, 8000),
		BLOOD_RIFT_GUARDIAN(7350, 7337, 20691, -1, Skill.RUNECRAFTING, 8000);
