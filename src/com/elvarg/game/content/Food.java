...
/**
 * Represents food which can be consumed by the player
...
public class Food {
	...
	public static boolean consume(Player player, int item, int slot) {
		...
		if(player.getDueling().inDuel() && player.getDueling().getRules()[DuelRule.NO_FOOD.ordinal()]) {
			...
			return true;
		} else if(player.getCombat().isStunned()) {
		...
		if (food == Edible.KARAMBWAN) {
			...
			if (!player.getFoodTimer().elapsed(1200)) {
				return true;
			}
	...
	/**
	 * Represents all types of food currently
	 * available.
	 * 
	 * @author relex lawl
	 */
