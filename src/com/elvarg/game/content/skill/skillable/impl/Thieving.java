 ...
 * 
 * The Thieving skill allows a player to steal items
 * in the game. Either from npcs or objects.
 * 
...
public class Thieving extends ItemIdentifiers {
	...
	/**
	 * The {@link Animation} an npc will perform when attacking
	 * a pickpocket.
	 */
	private static final Animation NPC_ATTACK_ANIMATION = new Animation(401);

	/**
	 * The {@link Animation} the player will perform when blocking
	 * an attacking {@link NPC}.
	 */
	...
	public static final class Pickpocketing {
					...
					TaskManager.submit(new Task(2, player, false) {
						...
						protected void execute() {
							if(isSuccessful(player, pickpocket.get())) {
								//Get the loot..
								Item loot = pickpocket.get().getRewards()[Misc.getRandom(pickpocket.get().getRewards().length)].clone();
								...
								player.getPacketSender().sendMessage("You steal "+(loot.getAmount() > 1 ? Integer.toString(loot.getAmount()) : Misc.anOrA(name))+" "+name+".");
								...
								npc.forceChat((pickpocket.get() == Pickpocketable.MASTER_FARMER ? "Cor blimey, mate! What are ye doing in me pockets?" 
										: "What do you think you're doing?"));
								npc.performAnimation(NPC_ATTACK_ANIMATION);								
								player.getPacketSender().sendMessage("You fail to pick the pocket.");
								player.getCombat().getStunTimer().start(pickpocket.get().getStunTime());
								player.performGraphic(STUNNED_GFX);
								player.performAnimation(PLAYER_BLOCK_ANIMATION);
								player.getCombat().getHitQueue().addPendingDamage(new HitDamage(pickpocket.get().getStunDamage(), HitMask.RED));
								player.getMovementQueue().reset();
							}
							//Add pet..
		...
		/**
		 * Determines the chance of failure.
		 * method.
		 * 
		 * @param player
	...
	public static final class StallThieving {
		...
		/**
		 * Checks if we're attempting to steal from a stall
			...
			if(stall.isPresent()) {
				...
				if(player.getSkillManager().getCurrentLevel(Skill.THIEVING) >= stall.get().getReqLevel()) {
					...
					if(player.getClickDelay().elapsed(1000)) {
						...
						//Add items..
						Item item = stall.get().getRewards()[Misc.getRandom(stall.get().getRewards().length)];
						...
						for(StallDefinition stallDef : stall.get().getStalls()) {
							if(stallDef.getObjectId() == object.getId()) {
								if(stallDef.getReplacement().isPresent()) {
									TaskManager.submit(new TimedObjectReplacementTask(object, new GameObject(stallDef.getReplacement().get(), object.getPosition(), object.getType(), object.getFace()), stall.get().getRespawnTicks()));
								}
								break;
							}
						}
					...
					DialogueManager.sendStatement(player, "You need a Thieving level of at least "+Integer.toString(stall.get().getReqLevel())+" to do this.");
				}
				return true;
			}
			return false;
		...
		/**
		 * Represents a stall which can be stolen from
		 * using the Thieving skill.
		 * @author Professor Oak
		 */
		public enum Stall {
			BAKERS_STALL(new StallDefinition[]{new StallDefinition(11730, Optional.of(634)), }, 5, 16, 3, new Item(CAKE), new Item(CHOCOLATE_SLICE), new Item(BREAD)),
			CRAFTING_STALL(new StallDefinition[]{new StallDefinition(4874, Optional.empty()), new StallDefinition(6166, Optional.empty())}, 5, 16, 12, new Item(CHISEL), new Item(RING_MOULD), new Item(NECKLACE_MOULD)),
			MONKEY_STALL(new StallDefinition[]{new StallDefinition(4875, Optional.empty())}, 5, 16, 12, new Item(BANANA)),
			MONKEY_GENERAL_STALL(new StallDefinition[]{new StallDefinition(4876, Optional.empty())}, 5, 16, 12, new Item(POT), new Item(TINDERBOX), new Item(HAMMER)),
			TEA_STALL(new StallDefinition[]{new StallDefinition(635, Optional.of(634)), new StallDefinition(6574, Optional.of(6573)), new StallDefinition(20350, Optional.of(20349))}, 5, 16, 12, new Item(CUP_OF_TEA)),
			SILK_STALL(new StallDefinition[]{new StallDefinition(11729, Optional.of(634))}, 20, 24, 8, new Item(SILK)),
			WINE_STALL(new StallDefinition[]{new StallDefinition(14011, Optional.of(634))}, 22, 27, 27, new Item(JUG_OF_WATER), new Item(JUG_OF_WINE), new Item(GRAPES), new Item(EMPTY_JUG), new Item(BOTTLE_OF_WINE)),
			SEED_STALL(new StallDefinition[]{new StallDefinition(7053, Optional.of(634)),}, 27, 10, 30, new Item(POTATO_SEED, 12), new Item(ONION_SEED, 11), new Item(CABBAGE_SEED, 10), new Item(TOMATO_SEED, 9), new Item(SWEETCORN_SEED, 7), 
					new Item(STRAWBERRY_SEED, 5), new Item(WATERMELON_SEED, 3), new Item(BARLEY_SEED, 5), new Item(HAMMERSTONE_SEED, 5), new Item(ASGARNIAN_SEED, 5), new Item(JUTE_SEED, 5), new Item(YANILLIAN_SEED, 5), new Item(KRANDORIAN_SEED, 5),
					new Item(WILDBLOOD_SEED, 3), new Item(MARIGOLD_SEED, 4), new Item(ROSEMARY_SEED, 4), new Item(NASTURTIUM_SEED, 4)),
			FUR_STALL(new StallDefinition[]{new StallDefinition(11732, Optional.of(634)), new StallDefinition(4278, Optional.of(634))}, 35, 36, 17, new Item(GREY_WOLF_FUR)),
			FISH_STALL(new StallDefinition[]{new StallDefinition(4277, Optional.of(4276)), new StallDefinition(4707, Optional.of(4276)), new StallDefinition(4705, Optional.of(4276))}, 42, 42, 17, new Item(RAW_SALMON), new Item(RAW_TUNA)),
			CROSSBOW_STALL(new StallDefinition[]{new StallDefinition(17031, Optional.of(6984))}, 49, 52, 15, new Item(BRONZE_BOLTS, 6), new Item(BRONZE_LIMBS), new Item(WOODEN_STOCK)),
			SILVER_STALL(new StallDefinition[]{new StallDefinition(11734, Optional.of(634)), new StallDefinition(6164, Optional.of(6984)), }, 50, 54, 50, new Item(SILVER_ORE)),
			SPICE_STALL(new StallDefinition[]{new StallDefinition(11733, Optional.of(634)), new StallDefinition(6572, Optional.of(6573)), new StallDefinition(20348, Optional.of(20349)), }, 65, 81, 133, new Item(SPICE)),
			MAGIC_STALL(new StallDefinition[]{new StallDefinition(4877, Optional.empty()), }, 65, 100, 133, new Item(AIR_RUNE, 20), new Item(WATER_RUNE, 20), new Item(EARTH_RUNE, 20), new Item(FIRE_RUNE, 20), new Item(LAW_RUNE, 6)),
			SCIMITAR_STALL(new StallDefinition[]{new StallDefinition(4878, Optional.empty())}, 65, 100, 133, new Item(IRON_SCIMITAR)),
			GEM_STALL(new StallDefinition[]{new StallDefinition(11731, Optional.of(634)), new StallDefinition(6162, Optional.of(6984)), }, 75, 160, 133, new Item(UNCUT_SAPPHIRE), new Item(UNCUT_EMERALD), new Item(UNCUT_RUBY), new Item(UNCUT_DIAMOND)),
			;
		...
		public static final class StallDefinition {
			...
			/**
			 * The replacement object for when this stall
			 * temporarily despawns.
			 */
