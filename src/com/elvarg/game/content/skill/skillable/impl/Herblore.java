...
import com.elvarg.game.model.menu.CreationMenu.CreationMenuAction;
import com.elvarg.game.model.menu.impl.SingleItemCreationMenu;
...
public class Herblore extends ItemIdentifiers {
	...
	public static boolean makeUnfinishedPotion(Player player, int itemUsed, int usedWith) {
		if(itemUsed == VIAL_OF_WATER || usedWith == VIAL_OF_WATER) {
			...
			if(unfinished != null) {
				CreationMenu menu = new SingleItemCreationMenu(player, unfinished.getUnfPotion(), "How many potions would you like to make?", new CreationMenuAction() {
					@Override
	...
	public static boolean finishPotion(Player player, int itemUsed, int usedWith) {
		...
		if(finished.isPresent()) {
			int result = finished.get().getFinishedPotion();
			CreationMenu menu = new SingleItemCreationMenu(player, result, "How many potions would you like to make?", new CreationMenuAction() {
				...
				public void execute(int item, int amount) {
					ItemCreationSkillable skillable = new ItemCreationSkillable(Arrays.asList(new RequiredItem(new Item(finished.get().getItemNeeded()), true), new RequiredItem(new Item(finished.get().getUnfinishedPotion()), true)), 
							new Item(result), amount, Optional.of(new AnimationLoop(ANIMATION, 4)), 
	...
	public enum PotionDose {
		...
		public static Optional<PotionDose> forId(int itemId) {
			PotionDose potion = potions.get(itemId);
			if(potion != null) {
				return Optional.of(potion);
			}
			return Optional.empty();
