...
public class Cooking extends ItemCreationSkillable {
	...
	public Cooking(GameObject object, Cookable cookable, int amount) {
		super(Arrays.asList(new RequiredItem(new Item(cookable.getRawItem()), true)), new Item(cookable.getCookedItem()), amount, Optional.of(new AnimationLoop(ANIMATION, 4)), 
	...
	public void finishedCycle(Player player) {
		...
		if(success(player, 3, cookable.getLevelReq(), cookable.getStopBurn())) {
			...
			player.getPacketSender().sendMessage("You cook the "+ItemDefinition.forId(cookable.getRawItem()).getName()+".");
			player.getSkillManager().addExperience(Skill.COOKING, cookable.getXp());
		} else {
			player.getInventory().add(cookable.getBurntItem(), 1);
			player.getPacketSender().sendMessage("You burn the "+ItemDefinition.forId(cookable.getRawItem()).getName()+".");
		}
	}
