...
public class ConstructionData {
    ...
    public static final int[] DOORSPACEIDS = new int[]{15314, 15313, 15305, 15306, 15317,
            15315, 15316, 15308, 15307};

    public static final String[] TELESCOPE_MESSAGES = new String[]

            {
                    "Hmmm... Are the stars really small or are they just very far away?",
                    ...
                    "One of those stars has... Little stars moving around it. Interesting..."
            };
    public static final int TREASURE_CHEST_1 = 0, TREASURE_CHEST_2 = 1, TREASURE_CHEST_3 = 2,
            ARMOUR_CASE = 3, WARDROBE = 4, CAPE_RACK = 5, TOY_BOX_1 = 6, TOY_BOX_2 = 7, TOY_BOX_3 = 8, COSTUME_BOX_1 = 9,
            COSTUME_BOX_2 = 10, COSTUME_BOX_3 = 11,
            COIN_STORAGE = 12;
    ...
    public static final int LIT_CANDLE = 34, PLANK = 960, OAK_PLANK = 8778, TEAK_PLANK = 8780,
            ...
            BRM = 8451, BD = 8453, BBB = 8455, BTH = 8437, BNH = 8419, SBH = 8441,
            TH = 8443, FH = 8445, TFH = 8447, TBH = 8449, BONES = 526, SKULLS = 964,
            COIN = 995, BOW = 1929, RD = 1763, LAW_RUNE = 563, COSMIC_RUNE = 564, NATURE_RUNE = 561;
    ...
    public static int getXOffsetForObjectId(int objectId, int hsId, int rotation) {
        ...
        int finalXTile = 0 + getRotatedLandscapeChunkX(rotation,
                objectDefinition.getSizeY(), hs.xOffset, hs.yOffset, objectDefinition.getSizeX(), hs.getRotation(0));
        return finalXTile;
    }
    ...
    public static int getYOffsetForObjectId(int objectId, int hsId, int rotation) {
        ...
        int finalYTile = 0 + getRotatedLandscapeChunkY(hs.yOffset,
                objectDefinition.getSizeY(), rotation, objectDefinition.getSizeX(), hs.xOffset, hs.getRotation(0));
        return finalYTile;
    }
    ...
    public static int getYOffsetForObjectId(int objectId, int offsetX, int offsetY, int rotation, int objectRot) {
        ...
        int finalYTile = 0 + getRotatedLandscapeChunkY(offsetY,
                objectDefinition.getSizeY(), rotation, objectDefinition.getSizeX(), offsetX, objectRot);
        return finalYTile;
    }
    ...
    public static int getXOffsetForObjectId(int objectId, int offsetX, int offsetY,
        ...
        ObjectDefinition objectDefinition = ObjectDefinition.forId(objectId);
        int finalXTile = 0 + getRotatedLandscapeChunkX(rotation,
                objectDefinition.getSizeY(), offsetX, offsetY,
                objectDefinition.getSizeX(), objectRot);
        return finalXTile;
    }
    ...
    public static int getXOffsetForObjectId(int objectId, HotSpots hs,
        ...
        if (objectDefinition != null) {
            finalXTile = 0 + getRotatedLandscapeChunkX(rotation,
                    objectDefinition.getSizeY(), hs.getXOffset(), hs.getYOffset(),
                    objectDefinition.getSizeX(), hs.getRotation(0));
        } else {
            finalXTile = 0 + getRotatedLandscapeChunkX(rotation,
                    1, hs.getXOffset(), hs.getYOffset(),
                    1, hs.getRotation(0));
        }
        return finalXTile;
    }
    ...
    public static int getYOffsetForObjectId(int objectId, HotSpots hs,
        ...
        int finalYTile = 0;
        if (objectDefinition != null) {
            finalYTile = 0 + getRotatedLandscapeChunkY(hs.getYOffset(),
                    objectDefinition.getSizeY(), rotation, objectDefinition.getSizeX(), hs.getXOffset(), hs.getRotation(0));
        } else {
            finalYTile = 0 + getRotatedLandscapeChunkY(hs.getYOffset(),
                    0, rotation, 0, hs.getXOffset(), hs.getRotation(0));
        }
        return finalYTile;
    }
    ...
    public static int getPlankMakeCost(int plank) {
        if (plank == 1511)
            return 15000;
        if (plank == 1521)
            return 40000;
  ...
  /*  public static void doPlankMake(Player p, int log, int amount) {
        if (!(p.getAbsX() >= 3203 && p.getAbsY() >= 3438 && p.getAbsX() <= 3206 && p.getAbsY() <= 3441)) {
            p.sendMessage("You can't do that here.");
            return;
        }
        if (p.getInterfaceChecking().getWindowInterfaceId() != 39000) {
            return;
        }
        int cost = getPlankMakeCost(log);
        for (int i = 0; i < amount; i++) {
            if (!p.getActionAssistant().playerHasItem(log, 1)) {
                p.sendMessage("You don't have the required logs to do this");
                return;
            }
            if (!p.getActionAssistant().deleteCash(cost)) {
                p.sendMessage("You need " + cost + " coins to do this");
                return;
            }
            Server.getDatabaseConnection().executeUpdate(new MoneySinkUpdateQuery("Construction", cost));
            p.getActionAssistant().deleteItem(log, 1);

            if (log == 1511)
                p.getActionSender().addItem(960, 1);
            else if (log == 1521)
                p.getActionSender().addItem(8778, 1);
            else if (log == 6333)
                p.getActionSender().addItem(8780, 1);
            else if (log == 6332)
                p.getActionSender().addItem(8782, 1);
        }

    }*/

    public static int getRoomTypeForHotSpotId(int hotSpotId) {
        for (HotSpots hs : HotSpots.values()) {
            if (hs.getHotSpotId() == hotSpotId)
                return hs.getRoomType();
        }
        return -1;
    }

    public static enum Tablet {
        ENCHANT_LEVEL_1_JEWELLERY(8016, 7, 10, new int[][]{{COSMIC_RUNE, 1}, {WATER_RUNE, 3}}),
        ENCHANT_LEVEL_2_JEWELLERY(8017, 27, 37, new int[][]{{COSMIC_RUNE, 1}, {AIR_RUNE, 3}}),
        ENCHANT_LEVEL_3_JEWELLERY(8018, 49, 60, new int[][]{{COSMIC_RUNE, 1}, {FIRE_RUNE, 5}}),
        ENCHANT_LEVEL_4_JEWELLERY(8019, 57, 70, new int[][]{{COSMIC_RUNE, 1}, {EARTH_RUNE, 10}}),
        ENCHANT_LEVEL_5_JEWELLERY(8020, 68, 80, new int[][]{{COSMIC_RUNE, 1}, {EARTH_RUNE, 15}, {WATER_RUNE, 15}}),
        ENCHANT_LEVEL_6_JEWELLERY(8021, 87, 100, new int[][]{{COSMIC_RUNE, 1}, {EARTH_RUNE, 20}, {FIRE_RUNE, 20}}),
        BONES_TO_BANANAS(8014, 15, 20, new int[][]{{NATURE_RUNE, 1}, {EARTH_RUNE, 2}, {WATER_RUNE, 2}}),
        BONES_TO_PEACHES(8015, 60, 58, new int[][]{{NATURE_RUNE, 2}, {EARTH_RUNE, 4}, {WATER_RUNE, 4}}),
        VARROCK_TELEPORT(8007, 25, 35, new int[][]{{LAW_RUNE, 1}, {AIR_RUNE, 3}, {FIRE_RUNE, 1}}),

        FALADOR_TELEPORT(8009, 37, 40, new int[][]{{LAW_RUNE, 1}, {AIR_RUNE, 3}, {WATER_RUNE, 1}}),
        LUMBRIDGE_TELEPORT(8008, 31, 35, new int[][]{{LAW_RUNE, 1}, {AIR_RUNE, 3}, {EARTH_RUNE, 1}}),
        CAMELOT_TELEPORT(8010, 45, 55, new int[][]{{LAW_RUNE, 1}, {AIR_RUNE, 5}}),
        ARDOUGNE_TELEPORT(8011, 51, 60, new int[][]{{LAW_RUNE, 2}, {WATER_RUNE, 2}}),
        WATCHTOWER_TELEPORT(8012, 58, 70, new int[][]{{LAW_RUNE, 2}, {EARTH_RUNE, 2}}),
        TELEPORT_TO_HOUSE(8013, 40, 50, new int[][]{{LAW_RUNE, 1}, {AIR_RUNE, 1}, {EARTH_RUNE, 1}}),;
        private int magicLvl, magicXp, tabletId;
        private int[][] requiredItems;

        private Tablet(int tabletId, int magicLvl, int magicXp,
                       int[][] requiredItems) {
            this.setMagicLvl(magicLvl);
            this.setMagicXp(magicXp);
            this.setTabletId(tabletId);
            this.setRequiredItems(requiredItems);
        }

        public void make(final Player p, final int count) {
            /*p.getEventHandler().addEvent(new CycleEvent() {
				int ticks = 0;
				@Override
				public void execute(CycleEventContainer container) {
					if(Construction.getCurrentRoom(p).id() != STUDY)
						container.stop();
					if(p.levels[MAGIC] < getMagicLvl())
					{
						DialogueMethods.sendStatement(p, "You need a magic level of "+getMagicLvl()+" to make this tablet");
						container.stop();
						return;
					}
					String s = "";
					...
					{
						if(!p.getActionAssistant().playerHasItem(getRequiredItems()[i][0], getRequiredItems()[i][1]))
						{
							s += getRequiredItems()[i][1]+" x "+ItemDefinition.forID(getRequiredItems()[i][0]).name+" ";
						}
					}
					if(!p.getActionAssistant().playerHasItem(SOFT_CLAY, 1))
					{
						s += "1 x Soft Clay ";
					}
					if(!s.equalsIgnoreCase(""))
					{
						DialogueMethods.sendStatement(p, "You need these items to make this tablet:", s);
						container.stop();
						return;
					}
					p.getActionSender().sendWindowsRemoval();
					AnimationProcessor.addNewRequest(p, 713, 0);
					for(int i = 0; i < getRequiredItems().length; i++)
					{
						for(int j = 0; j < getRequiredItems()[i][1]; j++)
						{
							p.getActionAssistant().deleteItem(getRequiredItems()[i][0], 1);
						}
					}
					p.getActionAssistant().deleteItem(SOFT_CLAY, 1);
					p.getActionSender().addItem(getTabletId());
					if(ticks == count) {
						container.stop();
						return;
					}
            ...
            p.getPacketSender().sendMessage("This will come soon!");
        }
    ...
    public static enum Crest {
        ...
        public static Crest forID(int id) {
            for (Crest c : values()) {
                if (c.ordinal() == id)
                    return c;
            }
        ...
        public void make(int id, int objectId, Player p) {
            int toMake = -1;
            int lvlNeeded;
            if (id == 1193 && (objectId == 13717 || objectId == 13818))
                toMake = getSteelKite();
            if (id == 1201 && (objectId == 13717 || objectId == 13818))
                toMake = getRuneKite();
            if (id == 1157 && (objectId == 13716 || objectId == 13717 || objectId == 13718))
                toMake = getSteelHelm();
            if (id == 1163 && (objectId == 13716 || objectId == 13717 || objectId == 13718))
                toMake = getRuneHelm();
            if (id == 960 && objectId == 13718)
                toMake = getBanner();
            if (toMake == -1)
                return;
            if (!p.getInventory().contains(id))
                return;
            lvlNeeded = (id == 1193 || id == 1201) ? 38 : id == 960 ? 48 : 43;
            if (p.getSkillManager().getCurrentLevel(Skill.CRAFTING) < lvlNeeded) {
                p.getPacketSender().sendMessage("You need a crafting level of " + lvlNeeded + " to do this.");
                return;
            }
            p.getInventory().delete(id, 1);
            if (id == 960)
                p.getInventory().delete(CLOTH, 1);
            p.getInventory().add(toMake, 1);
            if (id == 960)
               p.performAnimation(new Animation(3657));
            if (id == 1163 || id == 1157)
            	p.performAnimation(new Animation(3655));
            if (id == 1193 || id == 1201)
            	p.performAnimation(new Animation(3656));
        }
        ...
        public int getSteelHelm() {
            int i = ordinal();
            return 8682 + (i * 2);
        }

        public int getSteelKite() {
            int i = ordinal();
            return 8746 + (i * 2);
        }

        public int getRuneHelm() {
            int i = ordinal();
            return 8464 + (i * 2);
        }

        public int getRuneKite() {
            int i = ordinal();
            return 8714 + (i * 2);
        }
    }

    public static enum ItemContainer {
        OAK_TREASURE_CHEST(TREASURE_CHEST_1, 18804, 65535, new int[][]{
                {7364, 7368}, {7362, 7366}, {7396, 7392, 7388,},
                {7394, 7390, 7386}, {2587, 2583, 2585, 2589},
                {2595, 2591, 2593, 2597}, {2631}, {2633}, {2637},
                {10396}, {10392}, {10394}, {10398}, {10316},
                {10318}, {10320}, {10322}, {10324}, {10412, 10414},
                {10404, 10406}, {10408, 10410},}),
        TEAK_TREASURE_CHEST(
                TREASURE_CHEST_2, 18806, 65535,
                new int[][]{{7319}, {7321}, {7323}, {7325}, {7327},
                        {7372, 7380}, {7370, 7378}, {2577},
                        {2605, 2599, 2601, 2603}, {2613, 2607, 2609, 2611},
                        {2645}, {2647}, {2649}, {10400, 10402},
                        {10416, 10418}, {2579}}),
        MAHOGANY_TREASURE_CHEST(
                TREASURE_CHEST_3, 18808, 65535, new int[][]{{7376, 7384},
                {7374, 7382}, {7398, 7399, 7400}, {2581},
                {2627, 2623, 2625, 2629}, {2619, 2615, 2617, 2621},
                {2639}, {2641}, {2643}, {2652},
                {2657, 2653, 2655, 2659}, {2665, 2661, 2663, 2667},
                {2673, 2669, 2671, 2675}, {3486, 3481, 3483, 3488},
                {10452, 10446, 10440, 10458, 10464, 10470},
                {10456, 10450, 10460, 10468, 10474, 10444},
                {10454, 10442, 10448, 10462, 10466, 10472},
                {11277}, {13101}, {21258}, {21260}, {21262},
                {21264}}),

        OAK_ARMOUR_CASE(ARMOUR_CASE, 18778, 2, new int[][]{
                {4071, 4069, 4070, 4072}, {4506, 4504, 4505, 4507},
                {4511, 4509, 4510, 4512}, {18708, 18706, 18707, 18709},
                {5554, 5553, 5555, 5556, 5557},
                {6131, 6133, 6135, 6143, 6149}, {6623, 6617, 6625, 6633},
                {5574, 5575, 5576}, {9672, 9674, 9676}}), TEAK_ARMOUR_CASE(
                ARMOUR_CASE, 18780, 4, new int[][]{
                {4071, 4069, 4070, 4072}, {4506, 4504, 4505, 4507},
                {4511, 4509, 4510, 4512},
                {18708, 18706, 18707, 18709},
                {5554, 5553, 5555, 5556, 5557},
                {6131, 6133, 6135, 6143, 6149},
                {6623, 6617, 6625, 6633}, {5574, 5575, 5576},
                {9672, 9674, 9676}}), MAHOGANY_ARMOUR_CASE(
                ARMOUR_CASE, 18782, 65535, new int[][]{
                {4071, 4069, 4070, 4072}, {4506, 4504, 4505, 4507},
                {4511, 4509, 4510, 4512},
                {18708, 18706, 18707, 18709},
                {5554, 5553, 5555, 5556, 5557},
        ...
        OAK_WARDROBE(WARDROBE, 18784, 1, new int[][]{
                {4089, 4091, 4093, 4095, 4097},
                {4099, 4101, 4103, 4105, 4107},
                {4109, 4111, 4113, 4115, 4117},
                {6137, 6139, 6141, 6147, 6153},
                {6918, 6916, 6920, 6922, 6924},
                {3385, 3387, 3389, 3391, 3393},
                {6109, 6106, 6107, 6108, 6110, 6111},
                {15039, 15040, 15041, 15043, 15044}}),
        OAK_CARVED_WARDROBE(
                WARDROBE, 18786, 2, new int[][]{
                {4089, 4091, 4093, 4095, 4097},
                {4099, 4101, 4103, 4105, 4107},
                {4109, 4111, 4113, 4115, 4117},
                {6137, 6139, 6141, 6147, 6153},
        ...
        TEAK_WARDROBE(
                ...
                {6109, 6106, 6107, 6108, 6110, 6111},
                {15039, 15040, 15041, 15043, 15044}}),
        TEAK_CARVED_WARDROBE(
                WARDROBE, 18790, 4, new int[][]{
                {4089, 4091, 4093, 4095, 4097},
                {4099, 4101, 4103, 4105, 4107},
                {4109, 4111, 4113, 4115, 4117},
                {6137, 6139, 6141, 6147, 6153},
        ...
        OAK_CAPE_RACK(CAPE_RACK, 18766, 0, new int[]{1052, 6568, 6570, 2414,
                4315, 9748, 9751, 9754, 9757, 9760, 9763, 9766, 9769, 9772,
                9775, 9778, 9781, 9784, 9787, 9790, 9793, 9796, 9799, 9802,
                9805, 9808, 9811, 9813, 18509, 19709, 10069, 10071, 15432,
                20767, 20771, 19111}),
        TEAK_CAPE_RACK(CAPE_RACK, 18767, 1,
                new int[]{1052, 6568, 6570, 2414, 4315, 9748, 9751, 9754,
                        9757, 9760, 9763, 9766, 9769, 9772, 9775, 9778, 9781,
                        9784, 9787, 9790, 9793, 9796, 9799, 9802, 9805, 9808,
                        9811, 9813, 18509, 19709, 10069, 10071, 15432, 20767,
                        20771, 19111}),
        MAHOGANY_CAPE_RACK(CAPE_RACK, 18768,
                5, new int[]{1052, 6568, 6570, 2414, 4315, 9748, 9751, 9754,
                9757, 9760, 9763, 9766, 9769, 9772, 9775, 9778, 9781,
                9784, 9787, 9790, 9793, 9796, 9799, 9802, 9805, 9808,
                9811, 9813, 18509, 19709, 10069, 10071, 15432, 20767,
                20771, 19111}),
        GILDED_CAPE_RACK(CAPE_RACK, 18769, 10,
                new int[]{1052, 6568, 6570, 2414, 4315, 9748, 9751, 9754,
                        9757, 9760, 9763, 9766, 9769, 9772, 9775, 9778, 9781,
                        9784, 9787, 9790, 9793, 9796, 9799, 9802, 9805, 9808,
                        9811, 9813, 18509, 19709, 10069, 10071, 15432, 20767,
                        20771, 19111}),
        MARBLE_CAPE_RACK(CAPE_RACK, 18770,
                65535, new int[]{1052, 6568, 6570, 2414, 4315, 9748, 9751,
                9754, 9757, 9760, 9763, 9766, 9769, 9772, 9775, 9778,
                9781, 9784, 9787, 9790, 9793, 9796, 9799, 9802, 9805,
                9808, 9811, 9813, 18509, 19709, 10069, 10071, 15432,
                20767, 20771, 19111}),
        MAGICAL_CAPE_RACK(CAPE_RACK,
                18771, 65535, new int[]{1052, 6568, 6570, 2414, 4315, 9748,
                9751, 9754, 9757, 9760, 9763, 9766, 9769, 9772, 9775,
                9778, 9781, 9784, 9787, 9790, 9793, 9796, 9799, 9802,
                9805, 9808, 9811, 9813, 18509, 19709, 10069, 10071,
                15432, 20767, 20771, 19111}),


        OAK_TOY_BOX(TOY_BOX_1, 18798, 65535, new int[]{1037, 1419, 4079, 4566,
                7927, 6856, 6857, 6858, 6859, 6860, 6861, 6862, 6863, 9920,
                9921, 9922, 9923, 9924, 9925, 10507, 11015, 11016, 11017,
                11018, 11789, 12634, 14600, 14602, 14603, 14605, 14713, 15352,
                15426,}),
        TEAK_TOY_BOX(TOY_BOX_2, 18800, 65535, new int[]{
                1037, 1419, 4079, 4566, 7927, 6856, 6857, 6858, 6859, 6860,
                6861, 6862, 6863, 9920, 9921, 9922, 9923, 9924, 9925, 10507,
                11015, 11016, 11017, 11018, 11789, 12634, 14600, 14602, 14603,
                14605, 14713, 15352, 15426,}),
        MAHOGANY_TOY_BOX(TOY_BOX_3,
                18802, 65535, new int[]{1037, 1419, 4079, 4566, 7927, 6856,
                6857, 6858, 6859, 6860, 6861, 6862, 6863, 9920, 9921,
                9922, 9923, 9924, 9925, 10507, 11015, 11016, 11017,
                11018, 11789, 12634, 14600, 14602, 14603, 14605, 14713,
                15352, 15426,}),

        OAK_CUSTUME_BOX(COSTUME_BOX_1, 18772, 2, new int[][]{
                {3057, 3058, 3059, 3060, 3061}, {6184, 6185}, {6188},
                {7594, 7592, 7593, 7595, 7596}, {6656, 6654, 6655, 6657},
                {6182, 6180, 6181}}),
        TEAK_CUSTUME_BOX(COSTUME_BOX_2, 18774,
                4, new int[][]{{3057, 3058, 3059, 3060, 3061},
                {6184, 6185}, {6188},
                {7594, 7592, 7593, 7595, 7596},
                {6656, 6654, 6655, 6657}, {6182, 6180, 6181}}),
        MAHOGANY_CUSTUME_BOX(
                COSTUME_BOX_3, 18776, 65535, new int[][]{
                {3057, 3058, 3059, 3060, 3061}, {6184, 6185},
                {6188}, {7594, 7592, 7593, 7595, 7596},
                {6656, 6654, 6655, 6657}, {6182, 6180, 6181}}),

        WOODEN_CRATE(COIN_STORAGE, 13283, 1, new int[]{995}),
        OAK_CRATE(COIN_STORAGE, 13285, 1, new int[]{995}),
        TEAK_CRATE(COIN_STORAGE, 13287, 1, new int[]{995}),
        MAHOGANY_CRATE(COIN_STORAGE, 13289, 1, new int[]{995}),
        MAGIC_CRATE(COIN_STORAGE, 13291, 1, new int[]{995}),;
        private int furnitureId, containAmount, type;
        private int[] containable;
        private boolean sets;
        private int[][] containableSets;

        private ItemContainer(int type, int furnitureId, int containAmount, int[] containable) {
            setType(type);
            this.setFurnitureId(furnitureId);
            this.setContainAmount(containAmount);
            this.setContainable(containable);
            setSets(false);
        }

        private ItemContainer(int type, int furnitureId, int containAmount, int[][] containableSets) {
            setType(type);
            this.setFurnitureId(furnitureId);
            this.setContainAmount(containAmount);
            this.setContainableSets(containableSets);
            setSets(true);
        }

        public static ItemContainer getContainerForId(int id) {
            for (ItemContainer i : values()) {
                if (i.furnitureId == id)
                    return i;
            }
            return null;
        ...
        public static ItemContainer getContainerForType(int type) {
            for (ItemContainer i : values()) {
                if (i.type == type)
                    return i;
            }
        ...
        public int[] getItemAt(int i) {
            try {
                if (isSets())
                    return getContainableSets()[i];
                else
                    return new int[]{getContainable()[i]};
            } catch (Exception e) {
                return null;
            }
        }

        public int[] getSet(int itemId) {
            if (!isSets())
                return null;
            for (int i = 0; i < getContainableSets().length; i++) {
                if (getContainableSets()[i][0] == itemId)
                    return getContainableSets()[i];
            }
            return null;
        }

        public int[][] getContainableSets() {
            return containableSets;
        }

        public void setContainableSets(int[][] containableSets) {
            this.containableSets = containableSets;
        }
        ...
        public void setContainable(int[] containable) {
        ...
        public int getContainAmount() {
            return containAmount;
        }

        public void setContainAmount(int containAmount) {
            this.containAmount = containAmount;
        }
    ...
    public static enum Craftable {
        CLOCKWORK(8792, -1, 8, new int[]{STEEL_BAR}),
        TOY_HORSEY(2520, -1, 10, new int[]{PLANK}),
        TOY_SOLDIER(7759, 3595, 13, new int[]{8792, PLANK}),
        WOODEN_CAT(10891, -1, 16, new int[]{PLANK}),
        TOY_DOLL(7763, 3596, 18, new int[]{8792, PLANK}),
        SEXTANT(2574, -1, 23, new int[]{STEEL_BAR}),
        WATCH(2575, -1, 28, new int[]{8792, STEEL_BAR}),
        TOY_MOUSE(7767, 3597, 33, new int[]{8792, PLANK}),
        CLOCKWORK_CAT(7771, -1, 85, new int[]{8792, PLANK}),;
        private int itemId, npcId, craftLevel, craftXp;
        private int[] items;

        private Craftable(int itemId, int npcId, int craftLevel,
                          int[] items) {
            this.craftXp = 15;
            this.itemId = itemId;
            this.npcId = npcId;
        ...
        public static Craftable forItemId(int itemId) {
            for (Craftable c : values())
                if (c.itemId == itemId || c.itemId + 2 == itemId && c.npcId != -1)
                    return c;
            return null;
        }

        public static Craftable forNpcId(int npcId) {
            for (Craftable c : values())
                if (c.npcId == npcId)
                    return c;
            return null;
       ...
       /* public static boolean spawn(Player p, Craftable c) {
            if (Areas.isInDuelArenaFight(new Location(p.getAbsX(), p.getAbsY()))) {
                p.sendMessage("You can't do that here.");
                return false;
            }
            if (c.npcId == -1)
                return false;
            if (!p.getActionAssistant().playerHasItem(c.itemId + 2))
                return false;
            if (p.toys.size() >= 3) {
                p.sendMessage("It's already hard enought to keep track of 3 of these toys.");
                return false;
            }
            int total = 0;
            for (Player p_ : PlayerManager.getSingleton().getPlayers()) {
                if (p_ == null)
                    continue;
                total += p_.toys.size();
            }
            if (total > 250) {
                p.sendMessage("There are already 150 toys in the game, wait for a little while.");
                return true;
            }
            p.getActionAssistant().deleteItem(c.itemId + 2, 1);
            ToyDoll npc = (ToyDoll) NPC.newTempNPC(c.npcId, p.absX, p.absY, p.getPosition().getZ());
            npc.setOwner(p);
            npc.setMapInstance(p.getMapInstance());
            npc.winds = 50;
            p.toys.add(npc);
            return true;
        }

        public static void make(Player p, Craftable c) {
            if (p.levels[Skills.CRAFTING] < c.craftLevel) {
                p.sendMessage("You need a crafting level of " + c.craftLevel + " to craft this");
                p.getActionSender().sendWindowsRemoval();
                return;
            }
            String message = "";
            for (int i : c.items) {
                if (!p.getActionAssistant().playerHasItem(i)) {
                    message += Item.getName(i) + ", ";
                }
            }
            if (!message.equals("")) {
                p.sendMessage("You need these items to do this: " + message.substring(0, message.length() - 2) + ".");
                p.getActionSender().sendWindowsRemoval();
                return;
            }
            for (int i : c.items)
                p.getActionAssistant().deleteItem(i, 1);
            p.getActionAssistant().addSkillXP(c.craftXp, Skills.CRAFTING);
            p.getActionSender().addItem(c.itemId);
            p.getActionSender().sendWindowsRemoval();
        }
*/
        public int getNpcId() {
    ...
    public static final Position YANILLE_TELE = new Position(2606, 3093);
    ...
    public static enum Portals {
        ...
        public String canBuild(Player p) {
            if (requiredItems == null) {
                ...
                while (it.hasNext()) {
                    ...
                    if (portal.getRoomX() == myTiles[0] - 1
                            ...
                            && portal.getRoomZ() == (p.inDungeon() ? 4 : p.getPosition().getZ())
                ...
                if (!found) {
                    ...
                    p.getPacketSender().sendObjectsRemoval(myTiles[0] - 1, myTiles[1] - 1, p.inDungeon() ? 4 : p.getPosition().getZ());
                    Construction.placeAllFurniture(p, myTiles[0] - 1, myTiles[1] - 1, p.inDungeon() ? 4 : p.getPosition().getZ());
                    p.getPacketSender().sendInterfaceRemoval();
                    return null;
                }
            }
            ...
            for (int i = 0; i < requiredItems.length; i++) {
                if (!p.getInventory().contains(new Item(requiredItems[i][0], requiredItems[i][1])))
                    list.add(ItemDefinition.forId(requiredItems[i][0]).getName() + "x" + requiredItems[i][1]);
            }
            if (list.size() != 0) {
                p.getPacketSender().sendMessage("You need these runes to direct there.");
                for (String s : list)
        ...
        public void build(Player p) {
            ...
            for (Portal portal : p.portals) {
                if (portal.getRoomX() == myTiles[0] - 1
                    ...
                    portal.setType(type);
                    found = true;
                    Server.getLogger().info(portal.getRoomX() + "/" + (myTiles[0] - 1) + " " + portal.getId() + "/" + p.portalSelected);
                }
            }
            if (!found) {
                ...
                portal.setRoomZ(p.inDungeon() ? 4 : p.getPosition().getZ());
                portal.setType(type);
    ...
    public enum RoomData {
        ...
        GARDEN(1856, 5064, ConstructionData.GARDEN, 1, 150000, new boolean[]{true, true, true, true}),
        PARLOUR(1920, 5112, ConstructionData.PARLOUR, 1, 150000, new boolean[]{true, true, true, false}),
        KITCHEN(1936, 5112, ConstructionData.KITCHEN, 5, 750000, new boolean[]{true, true, false, false}),
        DINING_ROOM(1952, 5112, ConstructionData.DINING_ROOM, 10, 750000, new boolean[]{true, true, true, false}),
        WORKSHOP(1920, 5096, ConstructionData.WORKSHOP, 15, 1000000, new boolean[]{false, true, false, true}),
        BEDROOM(1968, 5112, ConstructionData.BEDROOM, 20, 1000000, new boolean[]{true, true, false, false}),
        SKILL_ROOM(1928, 5104, ConstructionData.SKILL_ROOM, 25, 2000000, new boolean[]{true, true, true, true}),
        ...
        GAMES_ROOM(1960, 5088, ConstructionData.GAMES_ROOM, 30, 3000000, new boolean[]{true, true, true, false}),
        COMBAT_ROOM(1944, 5088, ConstructionData.COMBAT_ROOM, 32, 3000000, new boolean[]{true, true, true, false}),
        QUEST_ROOM(1960, 5104, ConstructionData.QUEST_ROOM, 35, 3000000, new boolean[]{true, true, true, true}),
        MENAGERY(1912, 5072, ConstructionData.MENAGERY, 37, 4000000, new boolean[]{true, true, true, true}),
        STUDY(1952, 5096, ConstructionData.STUDY, 40, 7000000, new boolean[]{true, true, true, false}),
        CUSTOME_ROOM(1968, 5064, ConstructionData.COSTUME_ROOM, 42, 7000000, new boolean[]{false, true, false, false}),
        CHAPEL(1936, 5096, ConstructionData.CHAPEL, 45, 7000000, new boolean[]{true, true, false, false}),
        PORTAL_ROOM(1928, 5088, ConstructionData.PORTAL_ROOM, 50, 15000000, new boolean[]{false, true, false, false}),
        FORMAL_GARDEN(1872, 5064, ConstructionData.FORMAL_GARDEN, 55, 11000000, new boolean[]{true, true, true, true}),
        THRONE_ROOM(1968, 5096, ConstructionData.THRONE_ROOM, 60, 22000000, new boolean[]{false, true, false, false}),
        OUBLIETTE(1904, 5080, ConstructionData.OUBLIETTE, 65, 22000000, new boolean[]{true, true, true, true}),
        PIT(1896, 5072, ConstructionData.PIT, 70, 1000000, new boolean[]{true, true, true, true}),
        DUNGEON_STAIR_ROOM(1872, 5080, ConstructionData.DUNGEON_STAIR_ROOM, 70, 1000000, new boolean[]{true, true, true, true}),
        TREASURE_ROOM(1912, 5088, ConstructionData.TREASURE_ROOM, 75, 37000000, new boolean[]{false, true, false, false}),
        CORRIDOR(1888, 5080, ConstructionData.CORRIDOR, 70, 1000000, new boolean[]{false, true, false, true}),
        JUNCTION(1856, 5080, ConstructionData.JUNCTION, 70, 1000000, new boolean[]{true, true, true, true}),
        ...
        ROOF_4_DOORS(1952, 5064, ConstructionData.ROOF, 0, 0, new boolean[]{true, true, true, true}),
        ROOF_3_DOORS(1944, 5072, ConstructionData.ROOF, 0, 0, new boolean[]{true, false, true, true}),
        ROOF_2_DOORS(1928, 5072, ConstructionData.ROOF, 0, 0, new boolean[]{true, false, true, false}),;
    ...
    public enum HotSpots {
        ...
        CHAPEL_RUG_1(7, 15273, 4, 1, 0, ConstructionData.CHAPEL, 22, new Dimension(1, 4)),
        CHAPEL_RUG_2(7, 15274, 4, 1, 0, ConstructionData.CHAPEL, 22, new Dimension(1, 4)),
        CHAPEL_RUG_3(7, 15274, 4, 1, 0, ConstructionData.CHAPEL, 22, new Dimension(1, 4)),
        ...
        DUNG_STAIR_RUG_1(7, 15379, 2, 2, 0, ConstructionData.DUNGEON_STAIR_ROOM, 22, new Dimension(5, 5)),
        DUNG_STAIR_RUG_2(7, 15378, 2, 2, 0, ConstructionData.DUNGEON_STAIR_ROOM, 22, new Dimension(5, 5)),
        DUNG_STAIR_RUG_3(7, 15377, 2, 2, 0, ConstructionData.DUNGEON_STAIR_ROOM, 22, new Dimension(5, 5)),


        DUNG_STAIR_ROOM_DOOR_1(92, 15329, 3, 1, 3, ConstructionData.DUNGEON_STAIR_ROOM),
        DUNG_STAIR_ROOM_DOOR_2(92, 15328, 4, 1, 3, ConstructionData.DUNGEON_STAIR_ROOM),
        DUNG_STAIR_ROOM_DOOR_3(92, 15326, 3, 6, 1, ConstructionData.DUNGEON_STAIR_ROOM),
        DUNG_STAIR_ROOM_DOOR_4(92, 15327, 4, 6, 1, ConstructionData.DUNGEON_STAIR_ROOM),
        /**
         * Pit
         */
        ...
        TREASURE_MONSTER(95, 15257, 2, 2, 0, ConstructionData.TREASURE_ROOM),
    ...
    public enum Furniture {
        ...
        CIDER_BARREL(13, 13569, 91, 12, 8240, new int[][]{{PLANK, 3}, {NAILS, 3}, {5763, 8}}, new int[][]{{Skill.COOKING.ordinal(), 14}}),
        ASGARNIAN_ALE_BARREL(13, 13570, 184, 14, 8241, new int[][]{{OAK_PLANK, 3}, {1905, 8}}, new int[][]{{Skill.COOKING.ordinal(), 24}}),
        GREENMANS_ALE_BARREL(13, 13571, 184, 26, 8242, new int[][]{{OAK_PLANK, 3}, {1909, 8}}, new int[][]{{Skill.COOKING.ordinal(), 29}}),
        DRAGON_BITTER_BARREL(13, 13572, 224, 36, 8243, new int[][]{{OAK_PLANK, 3}, {STEEL_BAR, 2}, {1911, 8}}, new int[][]{{
                Skill.COOKING.ordinal(), 39}}),
        CHEFS_DELIGHT_BARREL(13, 13573, 224, 48, 8244, new int[][]{{OAK_PLANK, 3}, {STEEL_BAR, 2}, {5755, 8}}, new int[][]{{
                Skill.COOKING.ordinal(), 54}}),
        ...
        WOODEN_WORKBENCH(24, 13704, 120, 17, 8375, new int[][]{{PLANK, 5}, {NAILS, 5}}),
        OAK_WORKBENCH(24, 13705, 140, 32, 8376, new int[][]{{OAK_PLANK, 5}}),
        STEEL_FRAMED_BENCH(24, 13706, 150, 46, 8377, new int[][]{{OAK_PLANK, 6}, {STEEL_BAR, 4}}),
        BENCH_WITH_VICE(24, 13707, 200, 62, 8378, new int[][]{{OAK_PLANK, 2}, {STEEL_BAR, 1}}, 13706),
        BENCH_WITH_A_LATHE(24, 13708, 250, 77, 8379, new int[][]{{OAK_PLANK, 2}, {STEEL_BAR, 1}}, 13707),
        ...
        ARMOUR_STAND(25, 13715, 440, 55, 8391, new int[][]{{OAK_PLANK, 8}, {LIMESTONE_BRICK, 1}}),

        PLUMING_STAND(26, 13716, 120, 16, 8392, new int[][]{{OAK_PLANK, 2}}),
        SHIELD_EASEL(26, 13717, 240, 41, 8393, new int[][]{{OAK_PLANK, 4}}),
        BANNER_EASEL(26, 13718, 440, 66, 8394, new int[][]{{OAK_PLANK, 8}, {CLOTH, 2}}),
        ...
        GILDED_DRESSER(27, 13168, 400, 74, 8051, new int[][]{{MAHOGANY_PLANK, 2}, {MOLTEN_GLASS, 2}, {GOLD_LEAF, 1}}),
        ...
        GILDED_FOUR_POSTER(28, 13154, 850, 60, 8037, new int[][]{{MAHOGANY_PLANK, 5}, {CLOTH, 2}, {GOLD_LEAF, 2}}),
        ...
        GILDED_WARDROBE(29, 13161, 600, 87, 8044, new int[][]{{MAHOGANY_PLANK, 3}, {GOLD_LEAF, 1}}),
        ...
        GILDED_CLOCK(30, 13171, 550, 85, 8054, new int[][]{{MAHOGANY_PLANK, 2}, {CLOCKWORK, 1}, {GOLD_LEAF, 1}}),


        RUNE_CASE_1(31, 13507, 190, 41, 8276, new int[][]{{TEAK_PLANK, 2}, {MOLTEN_GLASS, 2}}, new int[][]{{Skill.RUNECRAFTING.ordinal(), 14}}),
        RUNE_CASE_2(31, 13508, 212, 41, 8277, new int[][]{{TEAK_PLANK, 2}, {MOLTEN_GLASS, 2}}, new int[][]{{Skill.RUNECRAFTING.ordinal(), 44}}),
        ...
        MITHRIL_ARMOUR(34, 13491, 135, 28, 8270, new int[][]{{1159, 1}, {1121, 1}, {1085, 1}}, new int[][]{{Skill.SMITHING.ordinal(), 68}}),
        ADAMANT_ARMOUR(34, 13492, 150, 28, 8271, new int[][]{{1161, 1}, {1123, 1}, {1091, 1}}, new int[][]{{Skill.SMITHING.ordinal(), 88}}),
        RUNE_ARMOUR(34, 13493, 165, 28, 8272, new int[][]{{1163, 1}, {1127, 1}, {1093, 1}}, new int[][]{{Skill.SMITHING.ordinal(), 99}}),

        BASIC_DECORATIVE_ARMOUR_STAND(35, 34256, 135, 28, 8273, new int[][]{{4071, 1}, {4072, 1}, {4069, 1}}),
        DETAILED_DECORATIVE_ARMOUR_STAND(35, 34263, 150, 28, 8274, new int[][]{{4506, 1}, {4507, 1}, {4504, 1}}),
        INTRICATE_DECORATIVE_ARMOUR_STAND(35, 34280, 165, 28, 8275, new int[][]{{4511, 1}, {4512, 1}, {4509, 1}}),
        PROFOUND_DECORATIVE_ARMOUR_STAND(35, 34281, 180, 28, 18755, new int[][]{{18708, 1}, {18709, 1}, {18706, 1}}),

        OAK_STAIRCASE(36, 13497, 300, 27, 8249, new int[][]{{OAK_PLANK, 10}, {STEEL_BAR, 4}}),
        TEAK_STAIRCASE(36, 13499, 450, 48, 8252, new int[][]{{TEAK_PLANK, 10}, {STEEL_BAR, 4}}),
        SPIRAL_STAIRCASE(36, 13503, 600, 67, 8258, new int[][]{{TEAK_PLANK, 10}, {LIMESTONE_BRICK, 7}}),
        MARBLE_STAIRCASE(36, 13501, 750, 82, 8255, new int[][]{{MAHOGANY_PLANK, 5}, {MARBLE_BLOCK, 5}}),
        MARBLE_SPIRAL(36, 13505, 1050, 97, 8259, new int[][]{{TEAK_PLANK, 10}, {MARBLE_BLOCK, 10}}),

        OAK_STAIRCASE_1(37, 13498, 300, 27, 8249, new int[][]{{OAK_PLANK, 10}, {STEEL_BAR, 4}}),
        TEAK_STAIRCASE_1(37, 13500, 450, 48, 8252, new int[][]{{TEAK_PLANK, 10}, {STEEL_BAR, 4}}),
        SPIRAL_STAIRCASE_1(37, 13504, 600, 67, 8258, new int[][]{{TEAK_PLANK, 10}, {LIMESTONE_BRICK, 7}}),
        MARBLE_STAIRCASE_1(37, 13502, 750, 82, 8255, new int[][]{{MAHOGANY_PLANK, 5}, {MARBLE_BLOCK, 5}}),
        MARBLE_SPIRAL_1(37, 13506, 1050, 97, 8259, new int[][]{{TEAK_PLANK, 10}, {MARBLE_BLOCK, 10}}),
        /*
		 * get me hotspot ids
		 */
        CLAY_ATTACK_STONE(39, 13392, 100, 39, 8153, new int[][]{{SOFT_CLAY, 10}}),
        ATTACK_STONE(39, 13393, 200, 59, 8154, new int[][]{{LIMESTONE_BRICK, 10}}),
        MARBLE_ATTACK_STONE(39, 13394, 1200, 79, 8155, new int[][]{{MARBLE_BLOCK, 10}}),
        ...
        TEAK_PRIZED_CHEST(41, 13386, 460, 34, 8166, new int[][]{{TEAK_PLANK, 4}, {GOLD_LEAF, 4}}),
        MAHOGANY_PRIZED_CHEST(41, 13388, 550, 54, 8167, new int[][]{{MAHOGANY_PLANK, 4}, {GOLD_LEAF, 4}}),

        HOOP_AND_STICK(38, 13399, 120, 30, 8162, new int[][]{{OAK_PLANK, 4}}),
        DARTBOARD(38, 13400, 290, 54, 8163, new int[][]{{TEAK_PLANK, 3}, {STEEL_BAR, 1}}),
        ARCHERY_TARGET(38, 13402, 450, 81, 8164, new int[][]{{TEAK_PLANK, 6}, {STEEL_BAR, 3}}),
        ...
        BOXING_RING(44, 13126, 400, 32, 8023, new int[][]{{OAK_PLANK, 6}, {CLOTH, 4}}),
        FENCING_RING(44, 13133, 450, 41, 8024, new int[][]{{OAK_PLANK, 8}, {CLOTH, 6}}),
        COMBAT_RING(44, 13137, 500, 51, 8025, new int[][]{{TEAK_PLANK, 6}, {CLOTH, 6}}),
        RANGING_PEDESTALS(44, 13145, 600, 71, 8026, new int[][]{{TEAK_PLANK, 8}}),// wall type (0)
        BALANCE_BEAM(44, 13142, 700, 81, 8027, new int[][]{{TEAK_PLANK, 10}, {STEEL_BAR, 5}}),//ground dec type (22)
        ...
        LARGE_MAP(45, 13527, 500, 78, 8296, new int[][]{{MAHOGANY_PLANK, 4}, {8006, 1}}),

        SILVERLIGHT(46, 13519, 187, 42, 8279, new int[][]{{TEAK_PLANK, 2}, {2402, 1}}),
        EXCALIBUR(46, 13521, 194, 42, 8280, new int[][]{{TEAK_PLANK, 2}, {35, 1}}),
        DARKLIGHT(46, 13520, 202, 42, 8281, new int[][]{{TEAK_PLANK, 2}, {6746, 1}}),
        ...
        ELEMENTAL_SPHERE(57, 13660, 380, 54, 8352, new int[][]{{TEAK_PLANK, 3}, {567, 1}, {GOLD_LEAF, 1}}),
        CRYSTAL_OF_POWER(57, 13661, 690, 66, 8353, new int[][]{{MAHOGANY_PLANK, 2}, {567, 1}, {GOLD_LEAF, 1}}),
        ...
        OAK_MAGIC_WARDROBE(64, 18784, 500, 42, 9829, new int[][]{{OAK_PLANK, 4}}),
        CARVED_OAK_MAGIC_WARDROBE(64, 18786, 550, 51, 9830, new int[][]{{OAK_PLANK, 6}}),
        TEAK_MAGIC_WARDROBE(64, 18788, 650, 60, 9831, new int[][]{{TEAK_PLANK, 4}}),
        CARVED_TEAK_MAGIC_WARDROBE(64, 18790, 700, 69, 9832, new int[][]{{TEAK_PLANK, 6}}),
        MAHOGANY_MAGIC_WARDROBE(64, 18792, 750, 78, 9833, new int[][]{{MAHOGANY_PLANK, 4}}),
        GILDED_MAGIC_WARDROBE(64, 18794, 900, 87, 9834, new int[][]{{MAHOGANY_PLANK, 4}, {GOLD_LEAF, 1}}),
        MARBLE_MAGIC_WARDROBE(64, 18796, 1500, 96, 9835, new int[][]{{MARBLE_BLOCK, 4}}),

        OAK_CAPE_RACK(65, 18766, 2600, 54, 9817, new int[][]{{OAK_PLANK, 4}}),
        TEAK_CAPE_RACK(65, 18767, 2850, 63, 9818, new int[][]{{TEAK_PLANK, 4}}),
        MAHOGANY_CAPE_RACK(65, 18768, 400, 72, 9819, new int[][]{{MAHOGANY_PLANK, 4}}),
        GILDED_CAPE_RACK(65, 18769, 500, 81, 9820, new int[][]{{MAHOGANY_PLANK, 4}, {GOLD_LEAF, 1}}),
        MARBLE_CAPE_RACK(65, 18770, 850, 90, 9821, new int[][]{{MARBLE_BLOCK, 1}}),
        MAGIC_CAPE_RACK(65, 18770, 2000, 99, 9822, new int[][]{{MAGIC_STONE, 1}}),

        OAK_TOY_BOX(62, 18798, 850, 50, 9836, new int[][]{{OAK_PLANK, 2}}),
        TEAK_TOY_BOX(62, 18800, 900, 68, 9837, new int[][]{{TEAK_PLANK, 2}}),
        MAHOGANY_TOY_BOX(62, 18802, 950, 86, 9838, new int[][]{{MAHOGANY_PLANK, 2}}),

        OAK_FANCY_DRESS_BOX(61, 18772, 450, 44, 9823, new int[][]{{OAK_PLANK, 2}}),
        TEAK_FANCY_DRESS_BOX(61, 18774, 890, 62, 9824, new int[][]{{TEAK_PLANK, 2}}),
        MAHOGANY_FANCY_DRESS_BOX(61, 18776, 920, 80, 9825, new int[][]{{MAHOGANY_PLANK, 2}}),
        ...
        MARIGOLDS(75, 13444, 100, 71, 8214, new int[][]{{BM, 1}}),
        ROSES(75, 13448, 100, 76, 8215, new int[][]{{BR, 1}}),

        ROSEMARY(76, 13437, 70, 66, 8210, new int[][]{{BRM, 1}}),
        DAFFODILS(76, 13441, 100, 71, 8211, new int[][]{{BD, 1}}),
        BLUEBELLS(76, 13439, 122, 76, 8212, new int[][]{{BBB, 1}}),

        THORNY_HEDGE(77, 13456, 70, 56, 8203, new int[][]{{BTH, 1}}),
        NICE_HEDGE(77, 13459, 100, 60, 8204, new int[][]{{BNH, 1}}),
        SMALL_BOX_HEDGE(77, 13462, 122, 64, 8205, new int[][]{{SBH, 1}}),
        TOPIARY_HEDGE(77, 13465, 141, 68, 8206, new int[][]{{TH, 1}}),
        FANCY_HEDGE(77, 13468, 158, 72, 8207, new int[][]{{FH, 1}}),
        TALL_FANCY_HEDGE(77, 13471, 223, 76, 8208, new int[][]{{TFH, 1}}),
        TALL_BOX_HEDGE(77, 13474, 316, 80, 8209, new int[][]{{TBH, 1}}),
		 ...
		 * Oubliette room
		 */
        TENTACLE_POOL(85, 13331, 326, 71, 8303, new int[][]{{BOW, 20}, {COIN, 100000 * 100}}),
        SPIKES(85, 13334, 623, 65, 8302, new int[][]{{STEEL_BAR, 20}, {COIN, 50000 * 100}}),
        FLAME_PIT(85, 13337, 357, 77, 8304, new int[][]{{STEEL_BAR, 20}, {COIN, 50000 * 100}}),
        ROCNAR(85, 13373, 387, 83, 8305, new int[][]{{COIN, 150000 * 100}}),
        ...
        SKELETON_GUARD(87, 13366, 223, 70, 8131, new int[][]{{COIN, 50000 * 100}}),
        GUARD_DOG(87, 13367, 273, 74, 8132, new int[][]{{COIN, 75000 * 100}}),
        HOBGOBLIN(87, 13368, 316, 78, 8133, new int[][]{{COIN, 100000 * 100}}),
        BABY_RED_DRAGON(87, 13372, 387, 82, 8134, new int[][]{{COIN, 150000 * 100}}),
        HUGE_SPIDER(87, 13370, 447, 86, 8135, new int[][]{{COIN, 200000 * 100}}),
        TROLL(87, 13369, 1000, 90, 8136, new int[][]{{COIN, 1000000 * 100}}),
        HELLHOUND(87, 2715, 2236, 94, 8137, new int[][]{{COIN, 5000000 * 100}}),
        ...
        TORCH(90, 13341, 244, 84, 8129, new int[][]{{OAK_PLANK, 4}, {LIT_CANDLE, 4}}),
        SKULL_TORCH(90, 13343, 244, 84, 8130, new int[][]{{OAK_PLANK, 4}, {LIT_CANDLE, 4}, {SKULLS, 4}}),

        SPIKE_TRAP(91, 13356, 223, 72, 8143, new int[][]{{COIN, 50000 * 100}}),
        MAN_TRAP(91, 13357, 273, 76, 8144, new int[][]{{COIN, 75000 * 100}}),
        TANGLE_TRAP(91, 13358, 323, 80, 8145, new int[][]{{COIN, 100000 * 100}}),
        MARBLE_TRAP(91, 13359, 387, 84, 8146, new int[][]{{COIN, 150000 * 100}}),
        TELEPORT_TRAP(91, 13360, 470, 88, 8147, new int[][]{{COIN, 200000 * 100}}),
         ...
         * Pit
         */
        MINOR_PIT_TRAP(93, 39266, 304, 71, 18797, new int[][]{{COIN, 45000 * 100}, {554, 500}}),
        MAJOR_PIT_TRAP(93, 39268, 1000, 83, 18798, new int[][]{{COIN, 125000 * 100}, {554, 2500}}),
        SUPERIOR_PIT_TRAP(93, 39270, 1100, 96, 18799, new int[][]{{COIN, 850000 * 100}, {554, 4500}}),

        PIT_DOG(94, 39260, 200, 70, 18791, new int[][]{{COIN, 40000 * 100}}),
        PIT_OGRE(94, 39261, 234, 73, 18792, new int[][]{{COIN, 55000 * 100}}),
        PIT_PROTECTOR(94, 39262, 300, 79, 18793, new int[][]{{COIN, 90000 * 100}}),
        PIT_SCARABITE(94, 39263, 387, 84, 18794, new int[][]{{COIN, 150000 * 100}}),
        PIT_BLACK_DEMON(94, 39264, 547, 89, 18795, new int[][]{{COIN, 300000 * 100}}),
        PIT_IRON_DRAGON(94, 39265, 2738, 97, 18796, new int[][]{{COIN, 7500000 * 100}}),

        /**
         * Treasure room
         */
        //DEMON(95, 13378, 707, 75, 8138, new int[][] {{COIN, 500000}}),
        KALPHITE_SOLDIER(95, 13374, 866, 80, 8139, new int[][]{{COIN, 750000 * 100}}),
        TOK_XIL(95, 13377, 2236, 85, 8140, new int[][]{{COIN, 5000000 * 100}}),
        DAGANNOTH(95, 13376, 2738, 90, 8141, new int[][]{{COIN, 7500000 * 100}}),
        STEEL_DRAGON(95, 13375, 3162, 95, 8142, new int[][]{{COIN, 10000000 * 100}}),
        ...
        WINTUMBER_TREE(1, 19038, 0, 1, 10508, new int[][]{{10508, 1}}),;
        ...
        public static ArrayList<Furniture> forFurnitureId_list(int furnitureId) {
        ...
        public static ArrayList<Furniture> forRoom(int[] room) {
            ArrayList<Furniture> value = new ArrayList<Furniture>();
            for (Furniture f : values()) {
                if (f == null)
                    continue;
                HotSpots h = HotSpots.forHotSpotId(f.getHotSpotId());
                if (h == null)
                    continue;
                for (int i : room)
                    if (h.getRoomType() == i)
            ...
            Iterator<Furniture> it = value.iterator();
            while (it.hasNext()) {
                Furniture f = it.next();
                if (found.contains(f.getItemId()))
                    it.remove();
                else
                    found.add(f.getItemId());
            }
            found.clear();
            found = null;
            Collections.sort(value, new Comparator<Furniture>() {
                @Override
                public int compare(Furniture p1, Furniture p2) {
                    return p1.getLevel() - p2.getLevel(); // Ascending
                }
            });
            return value;
        }
