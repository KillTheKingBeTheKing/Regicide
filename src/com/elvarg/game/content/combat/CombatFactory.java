...
public class CombatFactory {
	...
	public static CombatMethod getMethod(Character attacker) {
		...
		if(attacker.isPlayer()) {
			...
			p.getCombat().setAmmunition(AmmunitionData.getFor(p));
			p.getCombat().setRangedWeaponData(RangedWeaponData.getFor(p));
	...
	public static boolean canReach(Character attacker, CombatMethod method, Character target) {
		...
		if (attacker.getMovementQueue().isMoving() && target.getMovementQueue().isMoving()) {
			distance += attacker.getMovementQueue().isRunToggled() ? 2 : 1;
		}
		
		if (attacker.getPosition().getDistance(target.getPosition()) > distance) {
			attacker.getMovementQueue().setFollowCharacter(target);
			return false;
		}
	...
	public static void executeHit(PendingHit qHit) {
		...
		if(attacker.isPlayer()) {
			...
			if(Misc.getRandom(10) <= 5) {
				...
				if (method.getCombatType() == CombatType.MELEE || p_.getCombat().getWeapon() == WeaponInterface.DART || p_.getCombat().getWeapon() == WeaponInterface.KNIFE || p_.getCombat().getWeapon() == WeaponInterface.THROWNAXE || p_.getCombat().getWeapon() == WeaponInterface.JAVELIN) {
	...
	public static boolean checkAmmo(Player player, int amountRequired) {
		...
		final RangedWeaponData rangedWeapon = player.getCombat().getRangedWeaponData();
		...
		if(rangedWeapon == RangedWeaponData.TOXIC_BLOWPIPE) {
