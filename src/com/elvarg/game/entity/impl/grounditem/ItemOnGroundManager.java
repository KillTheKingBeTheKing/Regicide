...
import com.elvarg.game.task.impl.GroundItemSequenceTask;
...
public class ItemOnGroundManager {
	 ...
	 * We need to send all the items related to that region.
	...
	public static void perform(Player player, ItemOnGround item, OperationType type) {
		...
		case CREATE:
			...
			break;
		default:
			throw new UnsupportedOperationException("Unsupported operation ("+type.toString()+")  on: "+item.toString());
		}
	}
	...
	/**
	 * Attempts to merge an item with one
	 * that already exists in the same position.
	 * 
	...
	public static boolean merge(ItemOnGround item) {
		...
		for (; iterator.hasNext(); ) {
			...
			if(item_ == null || item_.isPendingRemoval()
					|| item_.equals(item)) {
				continue;
			}
	...
	/**
	 * Deregisters the given {@link ItemOnGround} from the world
	 * by flagging it as deleted. The iterator in {@link GroundItemSequenceTask}
	 * will pick this up and remove it.
	 * @param item
	 */
	...
	/**
	 * A utility method which quickly registers a default
	 * {@link ItemOnGround} which goes global
	 * once the item's counter hits {@code STATE_UPDATE_DELAY}.
	 * 
	 * @param player
	 * @param item
	 * @return
	...
	public static ItemOnGround register(Player player, Item item) {
		return register(player, item, player.getPosition().copy());
	}
	...
	/**
	 * A utility method which quickly registers a default
	 * {@link ItemOnGround} which does not go global
	 * once the item's counter hits {@code STATE_UPDATE_DELAY}.
	 * 
	 * @param player
	 * @param item
	...
	public static void registerNonGlobal(Player player, Item item) {
		registerNonGlobal(player, item, player.getPosition().copy());
	}
	...
	/**
	 * A utility method used to find a {@link ItemOnGround} with the
	...
	/**
	 * A utitily method used to check if the given {@link Player}
	 * is the owner of the given {@link ItemOnGround}.
	 * @param player
	 * @param i
	 * @return
	 */
	...
	/**
	 * Represents the different types of packet-operations
	 * related to ground items that are currently supported.
	 * @author Professor Oak
	 *
	 */
